...
public class CompareService : ICompareService
{
    ...
    private static string GenerateAddColumnSql(ColumnSchema col, DatabaseType dbType)
    {
        ...
        string dataTypeStr = GetDataTypeString(col, dbType);
    ...
    public static string GenerateCreateTableSql(TableSchema table, DatabaseType dbType, bool includeSchema = false)
    {
        var sb = new System.Text.StringBuilder();
        string schema = includeSchema ? table.SchemaName : string.Empty;
        string tableName = table.Name;

        // Quote table name appropriately based on target database type
        string quotedTableName = dbType switch
        {
            DatabaseType.SqlServer => string.IsNullOrEmpty(schema) ? $"[{tableName}]" : $"[{schema}].[{tableName}]",
            DatabaseType.PostgreSql => string.IsNullOrEmpty(schema) ? $"\"{tableName}\"" : $"\"{schema}\".\"{tableName}\"",
            DatabaseType.MySql or DatabaseType.MariaDb => string.IsNullOrEmpty(schema) ? $"`{tableName}`" : $"`{schema}`.`{tableName}`",
            DatabaseType.Oracle => string.IsNullOrEmpty(schema) ? $"\"{tableName}\"" : $"\"{schema}\".\"{tableName}\"",
            _ => string.IsNullOrEmpty(schema) ? $"[{tableName}]" : $"[{schema}].[{tableName}]"
        };
        ...
        foreach (var col in table.Columns.OrderBy(c => c.OrdinalPosition))
        {
            ...
            string columnName = dbType switch
            {
                DatabaseType.SqlServer => $"[{col.Name}]",
                DatabaseType.PostgreSql => $"\"{col.Name}\"",
                DatabaseType.MySql or DatabaseType.MariaDb => $"`{col.Name}`",
                DatabaseType.Oracle => $"\"{col.Name}\"",
                _ => $"[{col.Name}]"
            };

            // Determine datatype representation
            string dataTypeStr = GetDataTypeString(col, dbType);

            // Handle Identity / Auto-increment for primary keys
            string identityStr = "";
            if (col.IsIdentity)
            {
                identityStr = dbType switch
                {
                    DatabaseType.SqlServer => " IDENTITY(1,1)",
                    DatabaseType.PostgreSql => " GENERATED BY DEFAULT AS IDENTITY",
                    DatabaseType.MySql or DatabaseType.MariaDb => " AUTO_INCREMENT",
                    _ => ""
                };
            }
            ...
            columnDefinitions.Add($"    {columnName} {dataTypeStr}{identityStr}{defaultValue} {nullability}");
        }

        // Add Primary Key constraint if columns are marked
        var primaryKeys = table.Columns.Where(c => c.IsPrimaryKey).OrderBy(c => c.OrdinalPosition).ToList();
        if (primaryKeys.Any())
        {
            var pkColumns = string.Join(", ", primaryKeys.Select(col => dbType switch
            {
                DatabaseType.SqlServer => $"[{col.Name}]",
                DatabaseType.PostgreSql => $"\"{col.Name}\"",
                DatabaseType.MySql or DatabaseType.MariaDb => $"`{col.Name}`",
    ...
    public static string GenerateCreateUserDefinedTableTypeSql(UserDefinedTableTypeSchema tableType, DatabaseType dbType)
    {
        var sb = new System.Text.StringBuilder();
        string schema = tableType.SchemaName;
        string typeName = tableType.Name;

        string quotedTypeName = dbType switch
        {
            DatabaseType.SqlServer => string.IsNullOrEmpty(schema) ? $"[{typeName}]" : $"[{schema}].[{typeName}]",
            _ => string.IsNullOrEmpty(schema) ? $"[{typeName}]" : $"[{schema}].[{typeName}]"
        };

        sb.AppendLine($"CREATE TYPE {quotedTypeName} AS TABLE (");

        var columnDefinitions = new List<string>();
        foreach (var col in tableType.Columns.OrderBy(c => c.OrdinalPosition))
        {
            if (col.IsComputed) continue;
            ...
            columnDefinitions.Add($"    {columnName} {dataTypeStr}{defaultValue} {nullability}");
        }

        // Add Primary Key constraint if columns are marked
        var primaryKeys = tableType.Columns.Where(c => c.IsPrimaryKey).OrderBy(c => c.OrdinalPosition).ToList();
        if (primaryKeys.Any())
        {
            var pkColumns = string.Join(", ", primaryKeys.Select(col => dbType switch
            {
    ...
    private static string GetDataTypeString(ColumnSchema col, DatabaseType dbType)
    {
        string dataType = col.DataType;
        
        bool typeSupportsLength =
        ...
        if (typeSupportsLength && col.MaxLength.HasValue && col.MaxLength.Value > 0)
        {
            if (dbType == DatabaseType.SqlServer && col.MaxLength == 1073741823)
            {
                return $"{dataType}(MAX)";
            }
            return $"{dataType}({col.MaxLength.Value})";
        }
        else if (typeSupportsLength && col.MaxLength == -1)
        {
            return dbType switch
            {
                DatabaseType.SqlServer => $"{dataType}(MAX)",
                DatabaseType.PostgreSql => "TEXT",
                DatabaseType.MySql or DatabaseType.MariaDb => "LONGTEXT",
                _ => dataType
            };
        }
        ...
        {
            return col.Scale.HasValue
                ? $"{dataType}({col.Precision.Value},{col.Scale.Value})"
                : $"{dataType}({col.Precision.Value})";
        }
